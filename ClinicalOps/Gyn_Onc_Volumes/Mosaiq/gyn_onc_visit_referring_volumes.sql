...
--      ,[actv_code]
 ...
 --     ,[actv_desc]	       
	 ...
	 ,RTRIM(ISNULL(ext.First_Name, '')) + ' ' + RTRIM(ISNULL(ext.Last_Name, '')) As ReferringMD
   	 ,Facility.Name as ReferringFacility
  ...
  LEFT JOIN mosaiq.dbo.Facility ON adm.Ref_From_FAC_ID=Facility.FAC_ID 
  LEFT JOIN mosaiq.dbo.[External] ext ON adm.Ref_Md_ID=ext.PNP_ID
